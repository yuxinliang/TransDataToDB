    ...
    class XMLList
	{
		...
		public attribute(attributeName:any):XMLList
		{
			...
			for( var p in this)
			{
				...
				{
					list = (<XMLC> this[p] ).attribute(attributeName);
					rlt = rlt.concat(list);
				}
		...
		public child(propertyName:string):XMLList
		{
			...
			for( var p in this)
			{
				...
				list = (<XMLC> this[p] ).child(propertyName);
				rlt = rlt.concat(list);
			}
			var num:number = 0 ;
			var ret:XMLList = new XMLList();
		...
		public descendants(_name?:any):XMLList
		{
			...
			for( var p in this)
			{
				...
				{
					list = (<XMLC> this[p] ).descendants(_name);
					rlt = rlt.concat(list);
				}
		...
		public contains(_value:any):boolean
		{
			...
			for( var p in this)
			{
				if(this[p] && this[p].contains)
					rets = (<XMLC> this[p] ).contains(_value);
				if(rets == true)
					return true;
			}
			return rets;
		...
		public elements(name:any):XMLList
		{
			...
			for( var p in this)
			{
				...
				{
					list = (<XMLC> this[p] ).elements(name);
					rlt = rlt.concat(list);
				}
			}
			var num:number = 0 ;
