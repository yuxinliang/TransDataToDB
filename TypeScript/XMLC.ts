 ...
 */
    ...
    class XMLC {
        ...
        constructor(_value?:any, _vobj?:any, _parent?:any) {
            this.$value = _value;
            this.$valueObj = _vobj == undefined ? XML.parse(this.$value) : _vobj;
        }
         ...
         * @returns {XML}
         */
        public appendChild(child:any):XMLC {
            var temp:Object = XML.parse(child);
         ...
         * @returns {XML}
         */
        public prependChild(child:any):XMLC {
            var temp:Object = XML.parse(child);
        ...
        private freshen(xml:any):void {
            if (xml == null || !(xml instanceof XMLC))return;
            ...
            for (var p in list) {
                ...
                if (xml && xml.$valueObj && xml.$valueObj.hasOwnProperty("$" + _name) == true) {
                    if (_value == undefined || _value == "") {
                        if (!isexp)xml = new XMLC(xml.$valueObj["$" + _name], xml.$valueObj);
                        arr.push(xml)
                    }
                    else if (_exp == "==") {
         ...
         * @returns {XML}
         */
        private loopKey(_name:any, _vobj:Object, _con:Array<any>):XMLC {
            var myxml:XMLC;
            ...
            if (_vobj["name"] != undefined && (_vobj["name"] == _name || "*" == _name)) {
                myxml = new XMLC(this.loop(_vobj), _vobj);
                _con.push(myxml);
            }
            return myxml;
        }
        ...
        public attribute(attributeName:any):XMLList {
            ...
            if (b) {
                ...
                xml = new XMLC(ret, this.$valueObj);
                xmlList = new XMLList();
                xmlList[0] = xml;
            }
            else {
        ...
        public attributes():XMLList {
            ...
            var ats:Array<any> = XML.getAttributes(this.$valueObj);
            ...
            if (propertyName == null || propertyName == "*") {
                if (this.$valueObj["children"] != undefined) {
                    ...
                    for (prop in childrens) {
                        xml = new XMLC(this.loop(childrens[prop]), childrens[prop]);
                        xml.$parent = new XMLC(this.loop(this.$valueObj), this.$valueObj);
                        //xmls.push(xml);
                        rxmlList[prop] = xml;
                    }
                }
                ...
                childrens = new Array();
                XML.findChildren(this.$valueObj, propertyName, childrens);
                ...
                if (childrens.length == 0) {
                    ...
                    for (prop in childrens) {
                        ...
                        if (childrens[prop].parent != undefined) {
                            xml.$parent = new XMLC(this.loop(childrens[prop].parent), childrens[prop].parent);
                        }
                        else {
                            xml.$parent = null;
                        }
        ...
        public contains(_value:any):boolean {
            ...
            if (typeof (_value) == "string") {
                _value = new XMLC(_value, XML.parse(_value));
            }
                ...
                if (this.$valueObj["children"] != undefined) {
                    for (var prop in this.$valueObj["children"]) {
                        if (m != undefined && this.$valueObj["children"][prop].localName == m) {
                            xml = new XMLC(this.loop(this.$valueObj["children"][prop]), this.$valueObj["children"][prop]);
                            if (this.$valueObj["children"][prop].parent != undefined) {
                                xml.$parent = new XMLC(this.loop(this.$valueObj["children"][prop].parent), this.$valueObj["children"][prop].parent);
                            }
                            else {
                                xml.$parent = null;
                            }
