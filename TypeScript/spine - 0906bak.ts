 ...
 */
class spine extends DBImportTemplate
{
    ...
    public convertToDBData(spineJson)
    {
       ...
       for (var i = 0; i < 1; i++) {
            ...
            if (data.hasOwnProperty("slots")) {
                ...
                //for(var i = 0;i<armature["slot"].length;i++){
                //    var obj = {};
                //    obj["name"] = armature["slot"][i]["name"];
                //    if(armature["slot"][i]["displayIndex"]){
                //        obj["displayIndex"] = armature["slot"][i]["displayIndex"];
                //    }
                //    this.armatureSlot.push(obj);
                //}
    ...
    {
        var _frame = {}
        var hasFirshFrame = false;
        var colorFirstFrame = false;
        ...
        var flagC = -1
        if(!hasFirshFrame || !colorFirstFrame){//如果没有第一帧，添加第一帧
